...
This folder contains the reference instructions for each phase of the **platonic-coding** workflow. Always show the **current phase** when executing a step.
...
| 1 | `phase-1-rfc-spec.md` | Generate RFC from draft, then **platonic-code-specs** refine |
...
2. **Phase 1**: Follow `phase-1-rfc-spec.md`. Optionally ask for RFC index. Generate RFC, then call **platonic-code-specs** (refine). Output: RFC in `docs/specs/`.
...
- **Phase 1**: `platonic-code-specs` — refine operation (see `skills/platonic-code-specs/references/refine-specs.md`).
